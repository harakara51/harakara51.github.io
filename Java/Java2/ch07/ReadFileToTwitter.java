...

{
	...
	public static String tweettester;
	...
	public static void toTweetSize()
	{
		...
		while (counter < lineStore3.length)
		{
			...
			for (int i =0; i <(lineStore3.length-3); i ++)
			{
				...
				counter++;
				tweettester =tweet.toString();
				if (counter < (lineStore3.length-2))
				{
						tweettester= (tweettester + lineStore3[counter]);
				}
				if (counter > (lineStore3.length-2))
				{
				...
				if (tweet.append(lineStore3[counter]).length() > 139)
				{
	...
	public static void printTweet()
	{
		...
		System.out.println("Tweet " + num + " of " + (tweetholder.size() +1) + " Length of tweet " + lastTweet.length());
